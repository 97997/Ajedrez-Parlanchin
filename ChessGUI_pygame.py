	...
	def GetPlayerInput(self,board,currentColor):
		...
		while not fromSquareChosen or not toSquareChosen:
			...
			if e.type is MOUSEBUTTONDOWN:
				...
				##
				print squareClicked
				coordenada = TTS.coordToText(squareClicked)
				TTS.hablar(coordenada)
				##Da la coordenada
			...
			if not fromSquareChosen and not toSquareChosen:
				...
				if squareClicked != []:
					...
					print board[r][c]
					try:
						piezaTexto = TTS.pieceTypeToText(board[r][c][1]) + TTS.pieceColorToText(board[r][c][0])
					except:
						piezaTexto = "casilla vacia"
					print piezaTexto
					TTS.hablar(piezaTexto)
					piezaTexto = ""
