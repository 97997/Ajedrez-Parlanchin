...
class ChessBoard:
	...
	def MovePiece(self,moveTuple):
		...
		if toPiece == 'e':
			messageString = fromPiece_fullString + " se movio de " + self.ConvertToAlgebraicNotation(moveTuple[0]) + \
							" a " + self.ConvertToAlgebraicNotation(moveTuple[1])
			TTS.hablar(messageString)
			#Movimiento de pieza

		else:
			messageString = fromPiece_fullString+ " en "+self.ConvertToAlgebraicNotation(moveTuple[0])+\
						" captura al "+toPiece_fullString+" en "+self.ConvertToAlgebraicNotation(moveTuple[1])+"!"
			TTS.hablar(messageString)
		#capturar
